...
public abstract class SoDBoDPropertyTransformer<P extends AGPropertyEnforcementTransformerProperties, T extends SoDBoDPropertyTransformer<P,T>> extends ActivityGroupPropertyEnforcementTransformer<P,T> {
    ...
    public SoDBoDPropertyTransformer(P properties) throws PropertyException {
        super(properties);
    }
    ...
    /**
     * Checks if the property is ensured/violated trivially by the original
     * entries.<br>
     *
     * @param entries Entries before applying routines for property
    ...
    /**
     * Tries to choose originators for entries in a way that the property is
     * satisfied.<br>
     * That means that the set of originators for any two different activities
     * must not have common originators.
     *
     * @param candidateList
     * @param transformerAction
     * @return 
     */
    protected Map<SimulationLogEntry, Integer> findValidIndexCombination(Map<SimulationLogEntry, List<String>> candidateList, TransformerAction transformerAction) {
        ...
        IndexCounter<SimulationLogEntry> counter = new IndexCounter<>();
