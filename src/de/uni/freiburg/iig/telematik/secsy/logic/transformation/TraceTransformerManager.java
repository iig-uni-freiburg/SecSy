...
public class TraceTransformerManager {
        ...
        protected Map<AbstractTraceTransformer, List<Long>> succesfulAppliances = new HashMap<>();
        protected Map<AbstractTraceTransformer, List<Long>> unsuccesfulAppliances = new HashMap<>();

        protected LogEntryGenerator source;
        private Integer traces = 0;

        private final TransformerListenerSupport transformerListenerSupport = new TransformerListenerSupport();
        ...
        public void addTransformer(AbstractTraceTransformer traceTransformer) throws MissingRequirementException {
                ...
                if (source != null) {
                        for (Object requiredEntryFieldObject : traceTransformer.requiredEntryFields()) {
                                EntryField requiredEntryField = (EntryField) requiredEntryFieldObject;
                                if (!source.providesLogInformation(requiredEntryField)) {
                                        throw new MissingRequirementException(requiredEntryField);
                                }
        ...
        private void incSuccessfulAppliances(AbstractTraceTransformer transformer, long caseID) {
                if (!succesfulAppliances.containsKey(transformer)) {
                        succesfulAppliances.put(transformer, new ArrayList<>());
                }
                succesfulAppliances.get(transformer).add(caseID);
        }

        private void incUnsuccessfulAppliances(AbstractTraceTransformer transformer, long caseID) {
                if (!unsuccesfulAppliances.containsKey(transformer)) {
                        unsuccesfulAppliances.put(transformer, new ArrayList<>());
                }
